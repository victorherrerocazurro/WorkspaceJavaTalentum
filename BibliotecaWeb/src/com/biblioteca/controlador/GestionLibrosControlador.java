	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		case COMANDO_ALTA:
			...
			int numeroPaginas = Integer
							.parseInt(req.getParameter(NUM_PAGINAS));
			...
			try {
				...
				RequestDispatcher dispatcher 
							= req.getRequestDispatcher("/alta.jsp");
				//7-Enviar el feedback del Negocio a la Vista
				req.setAttribute(ATR_KEY_MENSAJE, 
						"El libro se ha dado de alta correctamente");
				//8-Invocar la vista
				...
				//7-Enviar el feedback del Negocio a la Vista
				req.setAttribute(ATR_KEY_MENSAJE, 
						"El libro no ha podido darse de alta");
				//8-Invocar la vista
				dispatcher.forward(req, resp);	
			}
			break;
		...
		case COMANDO_CONSULTA:
			...
			try {
				...
				//el resultado de la operación al cliente.
				RequestDispatcher dispatcher 
							= req.getRequestDispatcher("/listarLibros.jsp");
				//7-Enviar el feedback del Negocio a la Vista
				...
				//el resultado de la operación al cliente.
				RequestDispatcher dispatcher 
							= req.getRequestDispatcher("/consultaError.jsp");
				//7-Enviar el feedback del Negocio a la Vista
				req.setAttribute(ATR_KEY_MENSAJE, 
						"La consulta no ha sido resuelta correctamente");
				//8-Invocar la vista
				dispatcher.forward(req, resp);	
			}
		...
		default:
			RequestDispatcher dispatcher 
				= req.getRequestDispatcher("/operacionErronea.jsp");
			req.setAttribute(ATR_KEY_MENSAJE, 
					"La no es correcta solo se adminten alta y consulta");	
			dispatcher.forward(req, resp);	
			break;
		}
	}
